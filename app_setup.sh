#!/bin/bash

######################Customize Settings######################

#warName: The name of the war file generated by Maven build
warName=ninji-sample-1.0-SNAPSHOT.war

#dockerImageName: The name to build the docker image. (Do not need to change)
dockerImageName="webapp"
#containerName: The name to run the docker container. (Do not need to change)
containerName="myapp"
#appFolderName: The folder of your application.
appFolderName=ninji-sample


######################Customize Settings######################
oldwarName=ninji-sample-1.0-SNAPSHOT.war
olddockerImageName="webapp"
oldcontainerName="myapp"
oldappFolderName=ninji-sample

warPath=cp target/
appFolderPath=cd /usr/local/run/

if [ ! -d "app/run/$appFolderName" ]; then
	if [ ! -d "~/$appFolderName" ]; then
		if [ -f $appFolderName/.git ]; then
			sudo rm $appFolderName/.git
		fi
		mkdir app
		mkdir app/run
		mv $appFolderName app/run && echo "The application has been copied successfully."
	fi
fi

if [ ! -d "app/run/$appFolderName" ]; then
		echo "Fail to copy the application."
fi

sed -i -e "s/$warPath$oldwarName/$warPath$warName/g" start-script/start-everything.sh
sed -i -e "s/$appFolderPath$oldappFolderName/$appFolderPath$appFolderName/g" start-script/start-everything.sh
rm start-script/start-everything.sh-e


sed -i -e "s/$olddockerImageName/$dockerImageName/g" run.sh
sed -i -e "s/$oldcontainerName/$containerName/g" run.sh
rm run.sh-e

sed -i -e "s/$oldappFolderName/$appFolderName/g" Dockerfile
rm Dockerfile-e

sed -i -e "s/oldwarName=$oldwarName/oldwarName=$warName/g" customize.sh
sed -i -e "s/olddockerImageName=$olddockerImageName/olddockerImageName=$dockerImageName/g" customize.sh
sed -i -e "s/oldcontainerName=$oldcontainerName/oldcontainerName=$containerName/g" customize.sh
sed -i -e "s/oldappFolderName=$oldappFolderName/oldappFolderName=$appFolderName/g" customize.sh
rm customize.sh-e

sed -i -e "s/oldwarName=$oldwarName/oldwarName=$warName/g" customize.sh
sed -i -e "s/olddockerImageName=$olddockerImageName/olddockerImageName=$dockerImageName/g" customize.sh
sed -i -e "s/oldcontainerName=$oldcontainerName/oldcontainerName=$containerName/g" customize.sh
sed -i -e "s/oldappFolderName=$oldappFolderName/oldappFolderName=$appFolderName/g" customize.sh
rm customize.sh-e
